...
module CheckLinks
  class Checker
    ...
    def process_page(url : String, source_url : String)
      ...
      page = @main_cache.page_for_url(cache_key)
      if page.nil?
        page = Page.new(source_url)
        @main_cache.add_page_for_url(page, cache_key)
        page.open(url)
    ...
    def start_from(url : String)
      process_page(url)
      count = @main_cache._cache.reduce(0) { |count, (_, page)| page.exists? ? count + 1 : count }
      error_count = @main_cache._cache.reduce(0) { |count, (_, page)| page.xml_parsing_error? ? count + 1 : count }
      print "Found #{@main_cache.size} pages, #{count} exist, #{error_count} had HTML parsing error"

      @main_cache._cache.each do |url, page|
        next unless page.exists? && page.loaded?
        target_pages = page.links.map do |link|
          uri = resolved_uri(link, url)
          { uri, @main_cache.page_for_url(uri) }
        end
...
Checker.new.start_from("http://localhost:3000/")